...
public class EnvelopeWrappedSdkPublisherTest {
    ...
    private static final BiFunction<String, String, String> CONCAT_STRINGS_SLOWLY = (s1, s2) -> {
        try {
            Thread.sleep(10L);
    ...
    public void prefixOnly_noEvent() {
        ...
        fakePublisher.complete();
        verify(mockSubscriber).onNext("test-prefix:");
        verify(mockSubscriber).onComplete();
        verifyNoMoreInteractions(mockSubscriber);
    }
    ...
    public void prefixAndSuffix_noEvent() {
        ...
        fakePublisher.complete();
        verify(mockSubscriber).onNext("test-prefix::test-suffix");
        verify(mockSubscriber).onComplete();
        verifyNoMoreInteractions(mockSubscriber);
    }
    ...
    @Test
    public void prefixOnly_multipleSimultaneousEvents_onlyAppliesPrefixOnce() {
        EnvelopeWrappedSdkPublisher<String> contentWrappingPublisher =
            EnvelopeWrappedSdkPublisher.of(fakePublisher, "test-prefix:", null, CONCAT_STRINGS_SLOWLY);
        ...
        IntStream.range(0, 5).parallel().forEach(i -> fakePublisher.publish("test"));
        verify(mockSubscriber).onNext("test-prefix:test");
        verify(mockSubscriber, times(4)).onNext("test");
        verifyNoMoreInteractions(mockSubscriber);
        reset(mockSubscriber);
    ...
    @Test
    public void suffixAndPrefix_multipleSimultaneousCompletionsWithNoEvents_onlyPublishesOnce() {
        EnvelopeWrappedSdkPublisher<String> contentWrappingPublisher =
            EnvelopeWrappedSdkPublisher.of(fakePublisher, "test-prefix:", ":test-suffix", CONCAT_STRINGS_SLOWLY);
        ...
        IntStream.range(0, 5).parallel().forEach(i -> fakePublisher.complete());
        verify(mockSubscriber).onNext("test-prefix::test-suffix");
        verify(mockSubscriber, times(5)).onComplete();
        verifyNoMoreInteractions(mockSubscriber);
    }
